...
var Camera = function(aspect) {
    ...
    this.lookAt( vec3.fromValues(-(8*4), this.standing_height, -(8*4)), vec3.fromValues(0,0,0), vec3.fromValues(0,1,0) );
...
/**
 * This function performs movement on a copy of this.eye and returns
 * the new vec3 position.  This function is used in look-ahead.js
 * to assist in collision detection.  
 * 
 * @param {Number} delta The forward motion delta 
 * @param {vec3} original_position the original position vector of the movement
 * @return {vec3} a vec3 representing the new position of the camera
 */
Camera.prototype.checkWalk = function(delta, original_position) {
    let d = delta * 0.75;
    let new_position = vec3.create();
    vec3.copy(new_position, original_position);  //The new position starts as a copy of this.eye

    let n = vec3.create();
    vec3.cross(n, vec3.fromValues(this.rotation[0], this.rotation[4], this.rotation[8]), vec3.fromValues(0,1,0));
    vec3.scaleAndAdd(new_position, new_position, n, d);

    return new_position;    //Return the new position after the movement
}
 ...
 * @return {vec3} a vec3 representing the new position of the camera
 */
Camera.prototype.checkTrack = function(dx, dy, original_position) {
    dx = dx * 0.75;
    dy = dy * 0.75;
    ...
    let v = vec3.create();
    let vx = vec3.fromValues(this.rotation[0], this.rotation[4], this.rotation[8]);
    vec3.scaleAndAdd(new_position, new_position, vx, dx);

    let vy = vec3.fromValues(this.rotation[1], this.rotation[5], this.rotation[9]);
    vec3.scaleAndAdd(new_position, new_position, vy, dy);
