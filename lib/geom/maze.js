var Maze = {
    ...
    render: function( gl, uni ) {
        ...
        for(var i = 0; i < this.BOARDSIZE; i++){
            for(var j = 0; j < this.BOARDSIZE; j++){
                ...
                console.log("array eq. test: (\"true\" if worked): "+([0,0] === [0,0]));
    ...
    canCross: function(pos1,po2){

        let start = this.getQuad(pos1);
        let end = this.getQuad(pos2);
        let delta = [(start[0] - end[0]), (start[0] - end[0])];
        let ret = false;

        if(delta === [-1,0]){
            //Right
            if(this.board[start[0], start[1]].includes("E") || this.board[end[0], end[1]].includes("W") 
                                                            || this.board[end[0], end[1]].includes("F")){
                ret = false;
            }else{
                ret = true;
            }
            ...
            //Left
            if(this.board[start[0], start[1]].includes("W") || this.board[end[0], end[1]].includes("E")
                                                            || this.board[end[0], end[1]].includes("F")){
                ret = false;
            }else{
                ret = true;
            ...
            //Down
            if(this.board[start[0], start[1]].includes("S") || this.board[end[0], end[1]].includes("N")
                                                            || this.board[end[0], end[1]].includes("F")){
                ret = false;
            }else{
                ret = true;
            ...
            //Up
            if(this.board[start[0], start[1]].includes("N") || this.board[end[0], end[1]].includes("S")
                                                            || this.board[end[0], end[1]].includes("F")){
                ret = false;
            }else{
                ret = true;
